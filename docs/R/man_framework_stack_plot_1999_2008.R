...
data_decade_bef <- papers %>% 
  filter(pubyear > 1998 & pubyear < 2009)
data_all_bef <- data_decade_bef[,c("doi","pubyear")]  

######## Category #############

dictionary <- "Framework"
list_df <- expectation_functions(dictionary, data_all = data_decade_bef, num_cat=NULL, ini_cat=1,
  paper_cat_out=TRUE, filter_lines=TRUE, suffix="1999-2008",
...
Components_combo$lab_ypos[Components_combo$count < 50] <- NA
