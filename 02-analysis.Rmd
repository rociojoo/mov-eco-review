...
highest likelihood; see [here](R/02-TopicModelingScript_rj.R). 
...
Wordclouds of each topic based on $\hat{\beta}$ values. See the codes for the plot [here](R/02-wordcloud_plot.R).
</center>
<br />
...
<center>
![](posts/moveco-images/TopicsGamma_ts1.png){width=100%}

Time series of the relative prevalence of each topic every year. The code can be found [here](R/man_topics_trend_plot.R).
</center>
<br />
...
Heatmap of $\gamma$ values per abstract and topic. The corresponding code is [here](R/02-heatmap_plot.R).
</center>
<br />  
...
These wordclouds were overall consistent with the topic wordclouds, and gave
...
The code for this plot can be found [here](R/02-wordcloud_plot_II.R).
</center>
<br />
...
[4 highest-probability words](Data/Topics/word_intrusion/word_intrusion_real.csv), 
...
it to the group](Data/Topics/word_intrusion/word_intrusion_test.csv).
We asked 10 researchers in the field to identify the intruder in each group of 
words without any more explanations; their answers are
[here](Data/Topics/word_intrusion/).
...
The codes written for this analysis are [here](R/02-Word_Intrusion.R).
...
Topics 1, 2, 3, 5, 6, 7 and 12 got high scores. 
A few of the low-scored topics had a word intrusion that could be considered 
...
goal of the study is to predict over new data sets, which is not the case.

The word intrusion approach is not an exhaustive assessment of topic 
interpretability, but it allows putting our results into perspective. 
...
After having identified any taxon mentioned in a paper,  we summarized taxa at the Class level (except for superclasses Osteichthyes and Chondrichthyes which we merged into a single group labeled Fish, and for classes within the phylum Mollusca and the subphylum Crustacea which we considered collectively). Thus, each paper was classified as focusing on one or more class-like groups, as in @Holyoak2008: Fish, Mammals, Birds, Reptiles, Amphibians, Insects, Crustaceans, Mollusks, and others. For the purpose of our analysis, we kept humans as a separate category and did not count them within Class Mammalia. 
...
found [here](R/taxonomy_analysis.R).
...
More species have been studied in the last years. [Code for plot](R/02-stacked_species_plots.R).
</center>
<br /> 
...
Proportion of papers in each year studying each taxonomical group. [Code for plot](R/man_taxa_trend_plot.R).
</center>
<br /> 

<center>
![](posts/moveco-images/Barplots_topics_taxa.png){width=100%}

For each topic, relative frequencies of papers studying each taxonomical group. Only papers with more than 50% of association to each topic are used for this graph. [Code for plot](R/man_taxa_topic_barplot.R).
</center>
<br /> 
...
study of navigation. The framework dictionary is [here](Data/Dictionary/csv-updated-versions/Dictionaries-Framework.csv).
...
<center>
![](posts/moveco-images/MovEcoDesign-V.png){width=75%}
...
The size of each component box is proportional to the percentage of papers (in parentheses) tackling them irrespectful of whether they are only about this component or in combination with another one. The latter is specified through the segments that join the components to the observed movement path. One fill color corresponds to papers that only studied one component, while two or more colors correspond to papers that tackled two or three components, respectively (the ones from those colors). The width of the segment is proportional to the percentage of papers that studied that combination (or single component). Only combinations corresponding to $>5\%$ of papers are shown; e.g. combinations involving navigation and papers studying navigation on its own had $<5\%$ of papers each therefore they are not shown in the graph. This graph was made using Inkscape. The percentages in it and an alternative graphical representation of them can be found in [this code](R/man_framework_stack_plot_1999_2008.R).
</center>
<br /> 
...
Their use was assessed with a dictionary approach. The dictionary can be found [here](Data/Dictionary/csv-updated-versions/Dictionaries-Data.csv). 
...
<center>
![](posts/moveco-images/devices_ts_all.png){width=100%}

Proportion of papers of each year using each type of
device. [Code for plot](R/man_data_trend_plot.R).
</center>
<br /> 
...
The dictionary with the terms used can be found [here](Data/Dictionary/csv-updated-versions/Dictionaries-Software.csv). 
...
<center>
![](posts/moveco-images/software_ts_all.png){width=100%}

Proportion of papers of each year using each software. [Code for plot](R/man_software_trend_plot.R).
</center>
<br /> 
...
Within our dictionary approach, we first used expert opinion to compile all known statistical methods (and terms used for them) that could have been used in movement ecology, resulting in [204 terms](Data/Dictionary/csv-updated-versions/Dictionaries-Methods.csv). 
...
<center>
![](posts/moveco-images/method_ts.png){width=100%}

Proportion of papers of each year using each type of methods. [Code for plot](R/man_method_trend_plot.R). 
</center>
<br /> 
...
Table. Percentage of papers using each type of statistical method. The code for this table can be found [here](R/man_method_trend_plot.R). 
...
Table. Most common statistical trigrams in M&M sections of papers (with more than 100 mentions in papers). To reconstruct this table, readers can refer to [this code](R/trigrams.R). 


*The [code](R/02-Dictionary_Analysis.R) to process the dictionaries of the framework, tracking devices, software, statistical methods, and taxonomy is also available.
