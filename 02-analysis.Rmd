...
abstract was adressing. There are several variations of topic models (@Nunez-Mir2016). 
...
*tm* and *textstem* (@Rtextstem) were used in this stage.
...
dataset (@Waal2008). Basically, this quantity measures the degree
...
latent topics (@Chang2009). In fact, using this score could result into
having too many topics; see @Griffiths2004 who analyzed PNAS
...
[PASTE LINK TO CODES]   [MEB: These are all you]
...
Wordclouds of each topic based on $\beta$ values. [Code for wordcloud](R/02-wordcloud_plot.R)
</center>
<br />
...
Time series of the relative popularity of each topic every year. [Code for plot](R/man_topics_trend_plot.R)
</center>
<br />
...
Wordclouds of each topic based on most strongly associated abstracts. [Code for plot](R/02-wordcloud_plot.R)
[MEB: Im not really sure where the abstract world cloud is or is called in the code, since i never
made that you just used the code for the topics one I made. So i'm just copying
the code for the topics and you can build it yourself or take out the link entirely. Idc.]
</center>
<br />
...
Heatmap of $\gamma$ values per abstract and topic. [Code for the plot](R/02-heatmap_plot.R)
</center>
<br />  

All the codes to get the model outputs are in [PASTE LINK]. [MEB: The actual code is so disjunct and complicated and idk whats on or off or being called or not that i just made each one separately. You can do it yourself and remake one big giant commented code or just keep it as Ive made it. I honestly cant follow some of the code very well]
...
focusing on one or more taxa: Fish, Mammals, Birds, Reptiles, Amphibians,
...
[MEB: So These are not Classes, Fish sit in something like 5 classes, and sharks sit way out somewhere else, though most of the rest are. 
You told me these classifications were actually based on someones prior categories. 
We need to be explicit on this as we will not argue with people about the semantics and 'rightness' 
of these categories. Just say we're copying someones prior categories. And call them taxa not Classes.]
...
the dictionary was randomly selected to check if in those papers the categories
of the dictionary were correctly identified (i.e. accuracy). The accuracy
was XX%.  [Still waiting for Mathieu] [MEB: This is ridiculous, why dont you just do it yourself at this stage]
