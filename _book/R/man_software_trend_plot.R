...
soft_colors <- paletteer_d("colorblindr::OkabeIto") %>%
  as.vector()
names(soft_colors) <- unique(corrected_df$category)
soft_color_palette_current <- soft_colors[unique(corrected_df$category)]

ggplot(data  = corrected_df, mapping = aes(x = year, y = prop_papers, color = category, fill = category)) +
  geom_line(size=1.5, linetype = 3) +
  geom_point(size = 5.5) +
  scale_color_manual(name="Device",values = soft_color_palette_current) +
  scale_fill_manual(name="Device",values = soft_color_palette_current) +
