...
color.pallete<-brewer.pal(length(unique(corrected_df$category)), "Paired") # color palette


#ggplot(data  = corrected_df, mapping = aes(x = year, y = prop_papers, color = category)) +
...
# manual
linetypez <- c(1,2,4,3,5,6,5,3,4,2,1)
# or random
...
alphaz <- c(1,.4,.4,.4,.4,.4,.4,.4,.7,.9,1)
